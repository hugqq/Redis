...
public class TFollowServiceImpl extends ServiceImpl<TFollowMapper, TFollow> implements TFollowService {
    ...
    public Dict follow(Integer userId, Integer followUserId, Integer isFollowed) {
        ...
        TFollow follow = getOne(new LambdaQueryWrapper<TFollow>()
                .eq(TFollow::getUserId, userId)
                .eq(TFollow::getFollowUserId, followUserId)
        );
        // 如果没有关注信息，且要进行关注操作
        if (follow == null && isFollowed == 1) {
            ...
            if (flag) {
                ...
                return dict = Dict.create().set("code", "200").set("msg", "关注成功");
            } else {
                return dict = Dict.create().set("code", "500").set("msg", "关注失败");
            }
        }
        ...
        if (follow != null && follow.getIsFollowed() == 0 && isFollowed == 0) {
            ...
            if (flag) {
                ...
                return dict = Dict.create().set("code", "200").set("msg", "取关成功");
            } else {
                return dict = Dict.create().set("code", "500").set("msg", "取关失败");
            }
        }
        if (follow != null && follow.getIsFollowed() == 1 && isFollowed == 0) {
            return Dict.create().set("code", "500").set("msg", "重复取消关注");
        }
        if (follow != null && follow.getIsFollowed() == 0 && isFollowed == 1) {
            return Dict.create().set("code", "500").set("msg", "重复关注");
        }
